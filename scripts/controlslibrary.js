...
function OnPopupCustomLayoutMatchAccept() {
	ClearPopupsText();
	popupLoadingBarLevel = 0;
	const popup = UiToolkitAPI.ShowCustomLayoutPopupParameters("", "file://{resources}/layout/popups/popup_accept_match.xml", "map_and_isreconnect=de_dust2,false");
...
function OnPopupCustomLayoutWeaponUpdate() {
	...
	const defIndex = 23;
	UiToolkitAPI.ShowCustomLayoutPopupParameters("", "file://{resources}/layout/popups/popup_weapon_update.xml", defIndex, "none");
}

function OnPopupCustomLayoutSurvivalEndOfMatch() {
	UiToolkitAPI.ShowCustomLayoutPopupParameters("", "file://{resources}/layout/survival/survival_endofmatch.xml", "usefakedata=true", "none");
}
...
function OnRssFeedReceived(feed) {
	...
	feed.items.forEach(function (item) {
		const itemPanel = $.CreatePanel("Panel", RSSFeedPanel, "", { acceptsinput: true, onactivate: 'SteamOverlayAPI.OpenURL("' + item.link + '");' });
		itemPanel.AddClass("RSSFeed__Item");

		$.CreatePanel("Label", itemPanel, "", { text: item.title, html: true, class: "RSSFeed__ItemTitle" });
		if (item.imageUrl.length !== 0) $.CreatePanel("Image", itemPanel, "", { src: item.imageUrl, class: "RSSFeed__ItemImage", scaling: "stretch-to-fit-preserve-aspect" });
