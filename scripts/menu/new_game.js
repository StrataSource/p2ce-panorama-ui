...
var MainMenuNewGame = {
	onNewGameMenu() {
		...
		if (file) {
			for (let [_MenuIdentifier, MenuContents] of Object.entries(file)) {
				const button = $.CreatePanel("RadioButton", content, "", {
					class: "CampaignButtonButton ListItem MainNewGameListItem",
					group: "RBG1",
				});
				...
				$.CreatePanel("Label", button, "", {
					text: MenuContents.name,
					class: "CampaignButtonLabel CampaignButtonLabelPrimary",
				});
				$.CreatePanel("Label", button, "", {
					text: `\nBy ${MenuContents.publisher}`,
					class: "CampaignButtonLabel CampaignButtonLabelSecondary",
				});
				$.CreatePanel("Image", button, "", {
					src: MenuContents.icon,
					class: "CampaignButtonImage",
				});
			}
		} else {
			//TODO: Write proper error handling for when the kv file is invalid/missing.
			$.CreatePanel("Label", content, "", {
	...
	onNewgameSelected(maps) {
		...
		for (let [_MenuIdentifier, MenuContents] of Object.entries(maps)) {
			const button = $.CreatePanel("RadioButton", content, "", {
				class: "ListItem MainMenuChapterListItem",
				group: "RBG2",
			});
			button.RemoveAndDeleteChildren();
	...
	onChapterSelected(map) {
		...
		playbutton.SetPanelEvent("onactivate", () => MainMenu.playMap(map.map));
